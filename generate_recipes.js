...
const pasta_types = ["Spaghetti", "Penne", "Fusilli", "Rigatoni", "Farfalle", "Linguine", "Bucatini", "Paccheri", "Gnocchi", "Tagliatelle"];
const pasta_sauces = [
    ["al Pomodoro", [{name:"Passata di pomodoro",qty:500,unit:"ml"}, {name:"Cipolla",qty:1,unit:"pz"}]],
    ["al Ragù", [{name:"Macinato di carne",qty:400,unit:"g"}, {name:"Passata di pomodoro",qty:500,unit:"ml"}, {name:"Misto soffritto",qty:100,unit:"g"}]],
...
for (const pt of pasta_types) {
    for (const [ps, ings] of pasta_sauces) {
        const name = pt + " " + ps;
        const ingredients = [{name: pt, qty: 400, unit: "g"}, ...ings];
        recipes.push({id: "r_"+r_id, name, emoji: "🍝", cat: "Primi", portions: 4, ingredients});
        r_id++;
    }
}

for (const [rt, ings] of risotto_types) {
    const name = "Risotto " + rt;
    const ingredients = [{name: "Riso", qty: 320, unit: "g"}, {name: "Brodo", qty: 1, unit: "L"}, ...ings];
    recipes.push({id: "r_"+r_id, name, emoji: "🍚", cat: "Primi", portions: 4, ingredients});
    r_id++;
}

for (const [m_name, emoji, m_ings] of secondi_meats) {
    for (const [p_name, p_ings] of secondi_preparations) {
        const name = m_name + " " + p_name;
        const ingredients = [...m_ings, ...p_ings];
        recipes.push({id: "r_"+r_id, name, emoji, cat: "Secondi", portions: 4, ingredients});
        r_id++;
    }
}

for (const [f_name, emoji, f_ings] of secondi_fish) {
    for (const [p_name, p_ings] of secondi_preparations) {
        const name = f_name + " " + p_name;
        const ingredients = [...f_ings, ...p_ings];
        recipes.push({id: "r_"+r_id, name, emoji, cat: "Secondi", portions: 4, ingredients});
        r_id++;
    }
}

for (const [name, emoji, ings] of contorni) {
    recipes.push({id: "r_"+r_id, name, emoji, cat: "Contorni", portions: 4, ingredients: ings});
    r_id++;
}
for (const [name, emoji, ings] of piatti_unici) {
    recipes.push({id: "r_"+r_id, name, emoji, cat: "Piatti Unici", portions: 2, ingredients: ings});
    r_id++;
}
for (const [name, emoji, ings] of dolci) {
    recipes.push({id: "r_"+r_id, name, emoji, cat: "Dolci", portions: 6, ingredients: ings});
    r_id++;
}

const jsContent = "const RECIPES_DB = " + JSON.stringify(recipes, null, 2) + ";\n\n" +
...
fs.writeFileSync('c:/Users/stefa/Documents/GitHub/spesa-app/recipes-db.js', jsContent, 'utf-8');
console.log("Generato ricettario con " + recipes.length + " ricette!");
